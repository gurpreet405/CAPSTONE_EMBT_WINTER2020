 ...
 int main (void) // Main function
{
    ...
    if ((f1 = open("/dev/ttyO1", O_RDWR | O_NOCTTY | O_NONBLOCK))<0){ // Try opening the UART4 f1 in Read Write mod
...
sleep(1); // Wait for a second
    ...
    while(1)
    {
        ...
        strcpy(buf, "We are group 2 performimg ESP8266 Test\n"); // Copy a string in buf char array
        ...
        printf("\n\nBytes Received from ESP8266 - %d",incoming_data); // Print how many bytes was incomingbuffer[
        printf("\n");
